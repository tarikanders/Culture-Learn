...
export default function App() {
  ...
  // navTick increments on EVERY navigation click, even to the same mode.
  // FeedArea watches it to always escape the reading view when an nav item is tapped.
  const [navTick, setNavTick] = useState(0);

  const onNavigate = (m: Mode) => {
    setMode(m);
    setNavTick((t) => t + 1);
  };
    ...
    <div className="flex flex-col md:flex-row h-[100dvh] w-full bg-[#0A0A0B] text-[#F0F0F0] font-sans overflow-hidden">
      ...
      <main className="flex-1 relative flex flex-col h-full overflow-hidden pb-20 md:pb-0">
        <AnimatePresence mode="wait">
          ...
          >
            <FeedArea mode={mode} navTick={navTick} />
          </motion.div>
        </AnimatePresence>
      </main>

      <MobileNav mode={mode} onNavigate={onNavigate} />
    </div>
  );
}
